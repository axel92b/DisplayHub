...
RGBLEDmatrix::RGBLEDmatrix(char latchPin, char en_74138, char la_74138, char lb_74138, char lc_74138, char ld_74138,
                           char le_74138, char spiInterface, char ySize) {
...
void IRAM_ATTR RGBLEDmatrix::shiftOutRow() {
...
RedLEDmatrix::RedLEDmatrix(char latchPin, char en_74138, char la_74138, char lb_74138, char lc_74138, char ld_74138,
                           char le_74138, char spiInterface) {
    if (spiInterface == 0) // VSPI
    {
        SPI.begin(18, 19, 23, 5);
        SPI.setBitOrder(MSBFIRST);
...
void RedLEDmatrix::drawPixel(short x, short y) {
    ...
    if (color != BLACK_COLOR)
        currentFrameBuffer[y * maxX / 8 + x / 8] =
                currentFrameBuffer[y * maxX / 8 + x / 8] & ~(0x80 >> ((unsigned char) x % 8));
    else
        currentFrameBuffer[y * maxX / 8 + x / 8] =
                currentFrameBuffer[y * maxX / 8 + x / 8] | (0x80 >> ((unsigned char) x % 8));
}
...
short LEDmatrix::drawEngChar(short x, short y, char c) {
    Letter lt = engCharMap.getLetter(c);
    for (int i = 0; i < lt.height; i++) {
        for (int k = 0; k < lt.width; k++) {
            if (lt.pixels_rows[i][k]) {
                drawPixel(x + k, y + i);
            } else {
...
unsigned int LEDmatrix::getStringOnDisplayLength(const String &s) {
    return std::accumulate(s.begin(), s.end(), 0,
                           [this](int a, char b) { return a + this->engCharMap.getCharWidth(b) + 1; });
}
